...
const WordToPdf = () => {
  ...
  const handleConvert = async () => {
    ...
    try {
      ...
      setMessage(res.data.message || "Word converted to PDF with images and formatting intact.");
  ...
  return (
    <div className="max-w-3xl mx-auto my-8">
      ...
      <div className="text-center mb-8">
        <span className="inline-flex items-center gap-1.5 px-3.5 py-1 rounded-full text-xs font-semibold bg-violet-500/10 text-violet-400 border border-violet-500/20 mb-4 shadow-inner">
          <FileSpreadsheet className="w-3.5 h-3.5" /> High-Fidelity Converter
        </span>
        <h1 className="text-3xl sm:text-4xl font-extrabold font-heading text-white tracking-tight">
          Convert <span className="text-transparent bg-clip-text bg-gradient-to-r from-violet-400 via-purple-400 to-indigo-300">Word to PDF</span>
        </h1>
        <p className="text-slate-400 text-sm sm:text-base mt-2 max-w-lg mx-auto">
          Convert Word documents (.docx) to PDF format while preserving fonts, vectors, and embedded images seamlessly.
        </p>
      </div>
        ...
        {!download ? (
            ...
            <div
              ...
              className={`relative border-2 border-dashed rounded-2xl p-8 sm:p-12 text-center cursor-pointer transition-all duration-300 ${
                isDragging
                  ...
                  : "border-slate-700/80 hover:border-violet-500/60 bg-slate-900/40 hover:bg-slate-900/80"
              }`}
            >
              <input
                ref={fileInputRef}
              ...
              {!file ? (
                <div className="flex flex-col items-center">
                  <div className="w-16 h-16 rounded-2xl bg-gradient-to-tr from-violet-500/20 to-purple-500/20 border border-violet-500/30 flex items-center justify-center mb-4 group-hover:scale-110 transition-transform">
                  ...
                  <p className="text-slate-400 text-xs sm:text-sm mb-4">
                    or click to browse .docx files from your device
                  </p>
                  <span className="inline-flex items-center gap-1.5 px-4 py-2 rounded-xl bg-violet-600 hover:bg-violet-500 text-white text-xs font-semibold shadow-lg shadow-violet-600/30 transition-all">
            ...
            {isConverting && (
              <div className="mt-6 space-y-2">
                ...
                <div className="w-full h-2.5 bg-slate-800 rounded-full overflow-hidden p-0.5 border border-slate-700">
                  <div
                    className="h-full bg-gradient-to-r from-violet-500 via-purple-500 to-indigo-400 rounded-full transition-all duration-300 shadow-lg shadow-violet-500/50"
            ...
            <button
              ...
              className={`w-full py-4 mt-6 rounded-2xl font-bold font-heading text-sm sm:text-base flex items-center justify-center gap-2 shadow-xl transition-all duration-300 ${
                !file || isConverting
                  ...
                  : "bg-gradient-to-r from-violet-600 via-purple-600 to-indigo-600 hover:from-violet-500 hover:to-purple-500 text-white shadow-violet-600/30 hover:scale-[1.01] active:scale-[0.99]"
              }`}
            >
              {isConverting ? (
                <>
                ...
                <>
                  <Sparkles className="w-5 h-5" /> Convert Word to PDF (.pdf)
                </>
              )}
            </button>
          </div>
          ...
          <div className="text-center py-4 space-y-6">
            ...
            <div className="pt-4 border-t border-slate-800/80">
              <button
                onClick={resetForm}
                className="text-xs text-violet-400 hover:text-violet-300 font-medium inline-flex items-center gap-1.5 transition-colors"
              >
                <RefreshCw className="w-3.5 h-3.5" /> Convert another Word document
              </button>
            </div>
          </div>
        )}
      ...
      <div className="flex items-center justify-center gap-2 text-xs text-slate-500 mt-6 text-center">
        ...
        <span>DOCX images, headers, footers, and page margins are rendered precisely into PDF.</span>
      </div>
    </div>
  );
};
