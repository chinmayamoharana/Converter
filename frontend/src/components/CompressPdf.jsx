...
const CompressPdf = () => {
  ...
  const handleConvert = async () => {
    ...
    try {
      ...
      setMessage(res.data.message || "PDF compressed successfully.");
      setStats({
      ...
      const errorText = typeof rawErr === "string" ? rawErr : (typeof rawErr === "object" ? (rawErr.message || JSON.stringify(rawErr)) : "PDF compression failed. Please try another PDF.");
      setError(errorText);
      setMessage("");
    } finally {
      cleanupTimers();
  ...
  return (
    <div className="max-w-3xl mx-auto my-8">
      <div className="text-center mb-8">
        <span className="inline-flex items-center gap-1.5 px-3.5 py-1 rounded-full text-xs font-semibold bg-blue-500/10 text-blue-400 border border-blue-500/20 mb-4 shadow-inner">
          <Minimize2 className="w-3.5 h-3.5" /> PDF Optimizer
        </span>
        <h1 className="text-3xl sm:text-4xl font-extrabold font-heading text-white tracking-tight">
          Compress <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-400 via-cyan-400 to-teal-300">PDF File</span>
        </h1>
        <p className="text-slate-400 text-sm sm:text-base mt-2 max-w-lg mx-auto">
          Reduce PDF file size while preserving document quality for email and web sharing.
        </p>
      </div>
            ...
            >
              ...
              {!file ? (
                <div className="flex flex-col items-center">
                  <div className="w-16 h-16 rounded-2xl bg-gradient-to-tr from-blue-500/20 to-cyan-500/20 border border-blue-500/30 flex items-center justify-center mb-4 group-hover:scale-110 transition-transform">
                    <UploadCloud className="w-8 h-8 text-blue-400" />
                  </div>
                  <h3 className="text-lg font-bold font-heading text-white mb-1">
                    Drag & Drop PDF to compress
                  </h3>
                  <p className="text-slate-400 text-xs sm:text-sm mb-4">
                    or click to select PDF file from your device
                  </p>
                  <span className="inline-flex items-center gap-1.5 px-4 py-2 rounded-xl bg-blue-600 hover:bg-blue-500 text-white text-xs font-semibold shadow-lg shadow-blue-600/30 transition-all">
            ...
            {isConverting && (
              <div className="mt-6 space-y-2">
                ...
                <div className="w-full h-2.5 bg-slate-800 rounded-full overflow-hidden p-0.5 border border-slate-700">
                  <div
                    className="h-full bg-gradient-to-r from-blue-500 via-cyan-500 to-teal-400 rounded-full transition-all duration-300 shadow-lg shadow-blue-500/50"
            ...
            <button
              ...
              className={`w-full py-4 mt-6 rounded-2xl font-bold font-heading text-sm sm:text-base flex items-center justify-center gap-2 shadow-xl transition-all duration-300 ${
                !file || isConverting
                  ...
                  : "bg-gradient-to-r from-blue-600 via-cyan-600 to-blue-600 hover:from-blue-500 hover:to-cyan-500 text-white shadow-blue-600/30 hover:scale-[1.01]"
              }`}
            >
              {isConverting ? (
                <>
                  <RefreshCw className="w-5 h-5 animate-spin" /> Compressing PDF...
                </>
              ) : (
                <>
                  <Sparkles className="w-5 h-5" /> Compress PDF File
                </>
              )}
            </button>
          </div>
      ...
      <div className="flex items-center justify-center gap-2 text-xs text-slate-500 mt-6 text-center">
        ...
        <span>Optimizes PDF stream footprint for maximum size reduction.</span>
      </div>
    </div>
  );
};
